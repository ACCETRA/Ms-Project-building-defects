"""V2 improved YOLO training — augmentation, validation, patience, better hyperparameters."""
...
def main() -> None:
    ...
    parser.add_argument("--project", type=Path, default=ROOT / "runs/v2_improved/yolo")
    parser.add_argument("--name", default=None)
    args = parser.parse_args()

    # Apply task-aware defaults.
    if args.task == "segment":
        ...
        if args.name is None:
            args.name = "yolo11n_seg_v2"
    else:
        if args.manifest is None:
            args.manifest = _DETECT_MANIFEST
        if args.name is None:
            args.name = "yolo11n_detect_v2"
    ...
    class_ids = {
        "crack": 0, "spalling": 1, "honeycombing_rock_pocket": 2,
        "exposed_rebar": 3, "rust_staining": 4, "efflorescence_leaching": 5,
    }
    ...
    metadata: dict[str, Any] = {
        ...
        "improvements": [
            "val=True for per-epoch validation and best-model selection",
            "patience=15 for early stopping",
            "mosaic=1.0, mixup=0.15, copy_paste=0.1 for augmentation",
            "cos_lr=True for cosine learning rate schedule",
            "warmup_epochs=5 for stable early training",
            "label_smoothing=0.05 for regularization",
            "50 epochs (up from 20)",
        ],
    }
    ...
    try:
        model.train(
            ...
            cache=args.cache,
            # === V2 IMPROVEMENTS ===
            # Enable per-epoch validation — selects best checkpoint by mAP
            val=True,
            # Early stopping: stop if no improvement for 15 epochs
            patience=15,
            # Cosine learning rate schedule for smoother convergence
            cos_lr=True,
            # Longer warmup for stable early training
            warmup_epochs=5.0,
            # Augmentation: mosaic combines 4 images — great for multi-scale
            mosaic=1.0,
            # MixUp blends two images — improves generalization
            mixup=0.15,
            # Copy-paste augmentation for instance-level diversity
            copy_paste=0.1,
            # Label smoothing for regularization
            label_smoothing=0.05,
            # HSV augmentation for color robustness (cross-domain)
            hsv_h=0.015,
            hsv_s=0.7,
            hsv_v=0.4,
        ...
        # Same Windows apostrophe path-sanitizer workaround as v1
